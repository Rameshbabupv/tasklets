...
⚠️ **IMPORTANT:** These rules apply when Claude Code makes changes:

1. **Commit locally** with descriptive commit messages when work is complete
2. **NEVER push to remote automatically** - this requires explicit user permission
3. **Only push to remote** when you explicitly request it (e.g., "push to remote", "push this", "push to origin", etc.)
4. User reviews/approves changes before pushing
5. All commits must follow the pattern: proper message + `Co-Authored-By: Claude Haiku 4.5 <noreply@anthropic.com>`
