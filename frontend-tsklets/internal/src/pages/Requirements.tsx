...
export default function Requirements() {
  ...
  // Group by status
  const groupedRequirements: Record<RequirementStatus, Requirement[]> = {
    draft: [],
    brainstorm: [],
    solidified: [],
    approved: [],
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-7xl mx-auto p-8">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 className="text-4xl font-display font-bold bg-gradient-spark bg-clip-text text-transparent mb-2">
                  <span className="inline-block animate-float">📝</span> Requirements
                </h1>
                <p className="text-lg" style={{ color: 'var(--text-secondary)' }}>
                  Capture, brainstorm, and solidify requirements with Claude
                </p>
              </div>
              <motion.button
            ...
            <div className="flex gap-4 items-center flex-wrap">
              ...
              >
                ...
                <option value="solidified">Solidified</option>
              ...
              <div className="flex-1 min-w-[200px] relative">
                <input
                  ...
                  className="w-full px-4 py-2 pl-10 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all"
                  style={{ backgroundColor: 'var(--bg-card)', color: 'var(--text-primary)', borderColor: 'var(--border-primary)' }}
                />
                <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[20px]" style={{ color: 'var(--text-secondary)' }}>
                  search
                </span>
              </div>
            </div>
          ...
          {!loading && !selectedProduct && (
            <div className="text-center py-12">
              <span className="material-symbols-outlined text-6xl mb-4" style={{ color: 'var(--text-secondary)' }}>folder_open</span>
              <p className="text-lg" style={{ color: 'var(--text-secondary)' }}>Select a product to view requirements</p>
            </div>
          )}
          ...
          {!loading && selectedProduct && (
            <div className="space-y-8">
              {(Object.keys(groupedRequirements) as RequirementStatus[]).map(status => {
                const reqs = groupedRequirements[status]
                if (reqs.length === 0 && statusFilter !== 'all') return null
                ...
                return (
                  ...
                  >
                          ...
                          >
                            <Link to={`/requirements/${req.id}`}>
                              <motion.div
                                ...
                                className="p-6 rounded-xl border shadow-sm hover:shadow-md transition-all cursor-pointer"
                                style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-primary)' }}
                              >
                                <div className="flex items-start justify-between mb-3">
                                  <span className="text-xs font-mono font-semibold px-2 py-1 rounded bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300">
                                    {req.issueKey}
                                  </span>
                                  <span className={`text-xs font-bold px-2 py-1 rounded border ${priorityColors[req.priority]}`}>
                                    P{req.priority}
                                  </span>
                                </div>
                                ...
                                <div className="flex items-center gap-2 flex-wrap">
                                  {req.beadsEpicId && (
                                    <span className="text-xs px-2 py-1 rounded bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300 font-mono">
                                      Epic: {req.beadsEpicId}
                                    </span>
                                  )}
                    ...
                    {reqs.length === 0 && statusFilter === 'all' && (
                      <div className="text-center py-8 rounded-lg border-2 border-dashed" style={{ borderColor: 'var(--border-primary)' }}>
                        <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>No requirements in this status</p>
                      </div>
                    )}
                  </motion.div>
                )
